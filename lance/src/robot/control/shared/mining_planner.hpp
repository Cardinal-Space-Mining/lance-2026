...
enum class MiningDirection { UP, DOWN, LEFT, RIGHT };

inline std::string miningDirectionToString(MiningDirection dir) {
    switch (dir) {
        case MiningDirection::UP: return "UP";
        case MiningDirection::DOWN: return "DOWN";
        case MiningDirection::LEFT: return "LEFT";
        case MiningDirection::RIGHT: return "RIGHT";
...
public:
    ...
    DirectedMiningPath(MiningPath p, MiningDirection dir, const Eigen::MatrixXf* mat, const RobotParams& robot_params)
        : path(std::move(p)), direction(dir), matrix(mat), m_robot_params(robot_params) {  
    ...
    static float calculate_distance(const MiningPath& p, const Eigen::MatrixXf* mat) {
        if (mat == nullptr) return -1.0f;

        const auto& start = p.first;
        const auto& end   = p.second;

        if (end.x() != start.x()) {
            return std::abs(end.x() - start.x() + 1 - (1 - (*mat)(end.x(), end.y())));
        }
    ...
    bool check_validity() const {
        ...
        if (path.first.x() < 0 || path.first.y() < 0 || path.second.x() < 0 || path.second.y() < 0) return false;
        if (path.first.x() >= matrix->rows() || path.second.x() >= matrix->rows()) return false;
        if (path.first.y() >= matrix->cols() || path.second.y() >= matrix->cols()) return false;

        if (!(path.first.x() == path.second.x() || path.first.y() == path.second.y())) return false;
    ...
    float get_qualtiy(const Eigen::MatrixXi& previously_minined_locations) const {
        ...
        for (int i = p.first.x(); i <= p.second.x(); i++) {
            for (int j = p.first.y(); j <= p.second.y(); j++) {
                quality -= previously_minined_locations(i, j) * previously_mined_penalty;
            }
        }
    ...
    std::pair<Eigen::Vector2f,Eigen::Vector2f> get_path_coordinates_in_world_frame(float cell_size) const {
        MiningPath p = to_base_coordinates();
...
float const get_distance_for_path(const DirectedMiningPath& directed_path) {
    return DirectedMiningPath::calculate_distance(directed_path.path, directed_path.matrix);
}
...
public:
    MiningPlanner(RobotParams& robot_params) 
        ...
        MiningPlanner(std::function<float(float, float, float)> perception_eval, RobotParams& robot_params, Box2f mining_zone_bounds) :
            m_robot_params(robot_params),
            perception_eval(perception_eval), 
            mining_zone_bounds{mining_zone_bounds}
        {
            mapped_matrix_width = (int)(mining_zone_bounds.max().x() / robot_params.track_Width);
            mapped_matrix_height = (int)(mining_zone_bounds.max().y() / robot_params.track_Width);

            // UP/DOWN are (width, height) = (5, 4)
            strip_map_up = Eigen::MatrixXf::Zero(mapped_matrix_width, mapped_matrix_height);
            strip_map_down = Eigen::MatrixXf::Zero(mapped_matrix_width, mapped_matrix_height);
            
            // LEFT/RIGHT are swapped: (height, width) = (4, 5)
            strip_map_left = Eigen::MatrixXf::Zero(mapped_matrix_height, mapped_matrix_width);
            strip_map_right = Eigen::MatrixXf::Zero(mapped_matrix_height, mapped_matrix_width);

            times_mined_count_matrix = Eigen::MatrixXi::Zero(mapped_matrix_width, mapped_matrix_height);
        }
        ...
        DirectedMiningPaths final_output(){
            ...
            populate_planned_mining_paths(vertical_up_paths,strip_map_up,MiningDirection::UP);
            populate_planned_mining_paths(vertical_down_paths,strip_map_down,MiningDirection::DOWN);
            populate_planned_mining_paths(horizontal_left_paths,strip_map_left,MiningDirection::LEFT);
            populate_planned_mining_paths(horizontal_right_paths,strip_map_right,MiningDirection::RIGHT);

            DirectedMiningPaths all_mining_paths = vertical_up_paths;
            all_mining_paths.insert(all_mining_paths.end(), vertical_down_paths.begin(), vertical_down_paths.end());
            all_mining_paths.insert(all_mining_paths.end(), horizontal_left_paths.begin(), horizontal_left_paths.end());
            all_mining_paths.insert(all_mining_paths.end(), horizontal_right_paths.begin(), horizontal_right_paths.end());
        ...
        const Eigen::MatrixXf& get_strip_map(MiningDirection direction) const {
            ...
            switch (direction) {
                case MiningDirection::UP: return strip_map_up;
                case MiningDirection::DOWN: return strip_map_down;
                case MiningDirection::LEFT: return strip_map_left;
                case MiningDirection::RIGHT: return strip_map_right;
                default: return strip_map_up;
            }
        }
        void update_mapped_matrices() {
            ...
            populate_strip_map(strip_map_up, mapped_matrix_width, mapped_matrix_height, 
                            MiningDirection::UP, perception_eval);
            populate_strip_map(strip_map_down, mapped_matrix_width, mapped_matrix_height, 
                            MiningDirection::DOWN, perception_eval);
            populate_strip_map(strip_map_left, mapped_matrix_height, mapped_matrix_width, 
                            MiningDirection::LEFT, perception_eval);
            populate_strip_map(strip_map_right, mapped_matrix_height, mapped_matrix_width, 
                            MiningDirection::RIGHT, perception_eval);
        }
        
    private:
        void sort_paths_by_quality(DirectedMiningPaths& paths, const Eigen::MatrixXi& previously_mined_locations) {
        std::sort(paths.begin(), paths.end(), 
            [&previously_mined_locations](const DirectedMiningPath& a, const DirectedMiningPath& b) {
                return a.get_qualtiy(previously_mined_locations) > b.get_qualtiy(previously_mined_locations); 
            }
        );
        }

        void populate_planned_mining_paths(DirectedMiningPaths& dir_mining_paths, const Eigen::MatrixXf& mat, MiningDirection mining_dir) {
            int a = 0, b = 0;
            int width = mat.cols();
            int height = mat.rows();
            ...
            for (int i = 0; i < width; i++){
                ...
                while (b < height){
                    if (mat(a,i) == 0){
                        ...
                        MiningPath possible_path = std::pair(Eigen::Vector2i(a,i),Eigen::Vector2i(b-1,i));

                        DirectedMiningPath dir_mining_path(possible_path,mining_dir,&mat,m_robot_params);

                        if (get_distance_for_path(dir_mining_path) >= m_robot_params.min_zone_length){
        ...
        void remove_sections_for_robot_clearance(DirectedMiningPaths& dir_mining_paths) {
        ...
        void populate_strip_map(Eigen::MatrixXf& strip_map, int primary_dim, int secondary_dim, 
                        MiningDirection direction, std::function<float(float, float, float)> perception_eval) {
        float angle = (direction == MiningDirection::UP || direction == MiningDirection::DOWN) ? 90.0f : 0.0f;
        ...
        for (int i = 0; i < secondary_dim; i++) {
            ...
            while (row < primary_dim) {
                ...
                for (int j = row; j < primary_dim; j++) {
                    float distance_to_cell_end = (j - row + 1) * m_robot_params.track_Width;
                    if (distance >= distance_to_cell_end) {
                        strip_map(j, i) = 1.0f;
                    } else {
                        float distance_to_cell_start = (j - row) * m_robot_params.track_Width;
                        if (distance <= distance_to_cell_start) {
                            strip_map(j, i) = 0.0f;
                        } else {
                            strip_map(j, i) = (distance - distance_to_cell_start) / m_robot_params.track_Width;
                        }
                        break;
                    } 
                }
